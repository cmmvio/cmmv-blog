        ...
        if (response && response.posts) {
            posts.value = response.posts.map(post => {
                ...
                return {
                    ...
                    categoryNames: Array.isArray(post.categories) ? response.categories.map(category => post.categories.includes(category.id) ? category.name : null) : [post.categories?.name],
                    tags: post.tags || []
                }
            });
